...
export default function Overview({ user }: { user: User }) {
  ...
  const display = user.full_name || user.phone_number || 'pengguna'
  ...
  return (
    <div className="flex-1 overflow-y-auto px-8 py-8">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Halo, {display}.</h1>
          <p className="mt-1 text-sm text-ink-500">Ringkasan staf AI dan langganan Anda.</p>
        </div>
        <Link to="/app/arthur" className="btn-primary">Buat staf AI baru</Link>
      </div>
      ...
      <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat
          ...
          sub={maxAgents ? `Batas paket ${sub?.plan_label}` : undefined}
        />
        <Stat
          k="Token tersisa"
          v={nf(tokensLeft)}
          sub={tokensQuota ? `dari ${nf(tokensQuota)} kuota` : undefined}
        />
        <Stat k="Paket" v={sub?.plan_label ?? '—'} sub={sub?.status ?? undefined} />
        <Stat
          k="Berakhir"
      ...
      {sub && tokensQuota > 0 && (
        <div className="mt-6 surface p-5">
          <div className="flex items-center justify-between text-xs text-ink-500">
            <span>Pemakaian token bulan ini</span>
      ...
      <div className="mt-10">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-medium">Staf AI Anda</h2>
          <Link to="/app/agents" className="text-xs text-ink-700 hover:text-ink-900">Lihat semua</Link>
        </div>
        <div className="grid md:grid-cols-2 gap-4">
          ...
          {agents.length === 0 && (
            <div className="surface p-8 text-center text-sm text-ink-500 md:col-span-2">
              Belum ada staf AI. <Link to="/app/arthur" className="text-ink-900 underline">Mulai dengan Arthur</Link>.
            </div>
          )}
        </div>
      </div>
